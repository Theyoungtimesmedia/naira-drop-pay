...
-- Update deposits table with additional fields (skip existing columns)
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'deposits' AND column_name = 'client_ref') THEN
    ALTER TABLE public.deposits ADD COLUMN client_ref TEXT;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'deposits' AND column_name = 'gateway_ref') THEN
    ALTER TABLE public.deposits ADD COLUMN gateway_ref TEXT;
  END IF;
END $$;
